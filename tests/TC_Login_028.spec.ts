 ...
 * Test case: Verify that login fails with XSS injection attempt
 */
test('Verify Unsuccessful Login with XSS Injection Attempt', async ({ pageManager }) => {
    // 1. Navigate to login page
    await pageManager.getLoginPage().loadPage();
    await expect(pageManager.getLoginPage().getLoginForm()).toBeVisible();

    // 2. Enter XSS injection in email field and invalid password
    ...
    console.log('Login attempt with XSS injection in email field');
