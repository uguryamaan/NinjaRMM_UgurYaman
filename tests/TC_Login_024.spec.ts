 ...
 * Test case: Verify that login fails with empty username field
 */
test('Verify Unsuccessful Login with Empty Username Field', async ({ pageManager }) => {
    // 1. Navigate to login page
    await pageManager.getLoginPage().loadPage();
    await expect(pageManager.getLoginPage().getLoginForm()).toBeVisible();

    // 2. Leave email field empty and enter valid password
    const validPassword = DataManager.testData.getValidLoginBody().getPassword();
    await pageManager.getLoginPage().getPasswordInput().fill(validPassword);
    console.log('Login attempt with empty email and valid password');
